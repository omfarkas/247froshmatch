...
const ReviewMatches = () => {
  ...
  const [matches, setMatches] = useState([
    ...
    { id: 2, type: 'match', person1: 'Justin B', person2: 'Peter P', x: 50, y: 50, gender: 'male', rating: 3, notes: 'Both night owls, computer science majors', location: 'canvas' },
    { id: 3, type: 'match', person1: 'Binh L', person2: 'Chris P', x: 50, y: 250, gender: 'male', rating: 5, notes: 'Early risers, love hiking', location: 'sidebar' },
    { id: 4, type: 'match', person1: 'Sarah M', person2: 'Jessica K', x: 350, y: 50, gender: 'female', rating: 4, notes: 'Quiet study habits', location: 'canvas' },
    { id: 5, type: 'match', person1: 'Drew H', person2: 'William N', x: 650, y: 50, gender: 'male', rating: 2, notes: 'Different cleanliness standards', location: 'canvas' },
    { id: 6, type: 'match', person1: 'Emily R', person2: 'Anna T', x: 50, y: 450, gender: 'female', rating: 5, notes: 'Best friends from high school', location: 'sidebar' },
    { id: 7, type: 'match', person1: 'Marcus L', person2: 'David K', x: 50, y: 220, gender: 'male', rating: 4, notes: 'Both engineering majors, similar schedules', location: 'canvas' },
    { id: 8, type: 'match', person1: 'Sophia W', person2: 'Maya C', x: 350, y: 220, gender: 'female', rating: 5, notes: 'Pre-med students, study together often', location: 'canvas' },
    { id: 9, type: 'match', person1: 'Alex T', person2: 'Ryan M', x: 50, y: 50, gender: 'male', rating: 3, notes: 'Athletes, both on swim team', location: 'sidebar' },
    { id: 10, type: 'match', person1: 'Olivia B', person2: 'Emma S', x: 650, y: 220, gender: 'female', rating: 4, notes: 'Music majors, practice schedules align', location: 'canvas' },
    { id: 11, type: 'match', person1: 'Nathan G', person2: 'Kevin H', x: 50, y: 50, gender: 'male', rating: 5, notes: 'Both introverted, respect quiet time', location: 'sidebar' },
    { id: 12, type: 'match', person1: 'Rachel D', person2: 'Hannah P', x: 50, y: 390, gender: 'female', rating: 3, notes: 'Different majors but compatible schedules', location: 'canvas' },
    { id: 13, type: 'match', person1: 'Tyler J', person2: 'Brandon F', x: 50, y: 50, gender: 'male', rating: 4, notes: 'Both business majors, entrepreneurial interests', location: 'sidebar' },
    { id: 14, type: 'match', person1: 'Chloe V', person2: 'Zoe R', x: 350, y: 390, gender: 'female', rating: 5, notes: 'Roommates last year, want to continue', location: 'canvas' },
    { id: 15, type: 'match', person1: 'Daniel W', person2: 'Eric N', x: 650, y: 390, gender: 'male', rating: 3, notes: 'Both international students, similar backgrounds', location: 'canvas' },
  ]);
  ...
  const getRecommendedMatches = (profileId) => {
    ...
    return matches
      .filter(match => 
        match.gender === draggedProfile.gender && 
        !match.person3 // Only pairs, not triples
      )
  ...
  const handleProfileStop = (id, data) => {
    ...
    const draggedProfile = { ...profiles.find(p => p.id === id), x: data.x, y: data.y };
    ...
    if (absoluteX < SIDEBAR_WIDTH) {
      ...
      const sidebarMatches = matches.filter(m => m.location === 'sidebar' && !m.person3);
      ...
      if (sidebarRect) {
        ...
        if (targetMatchId) {
          ...
          if (targetMatch && !targetMatch.person3) {
            ...
            setMatches(matches.map(m => m.id === targetMatchId ? updatedMatch : m));
    ...
    if (nearbyMatches.length === 1 && nearbyProfiles.length === 0) {
      ...
      console.log('Adding profile to existing match:', draggedProfile.name, 'to', matchToExpand);
      ...
      setMatches(matches.map(m => m.id === matchToExpand.id ? expandedMatch : m));
    ...
    if (nearbyProfiles.length > 0 && nearbyProfiles.length <= 2) {
      ...
      console.log('Combining profiles:', profilesToCombine.map(p => p.name));
      
      // Calculate average position
      const avgX = profilesToCombine.reduce((sum, p) => sum + p.x, 0) / profilesToCombine.length;
      const avgY = profilesToCombine.reduce((sum, p) => sum + p.y, 0) / profilesToCombine.length;
      ...
      setProfiles(updatedProfiles.filter(p => !profileIdsToRemove.includes(p.id)));
  ...
  const handleMatchSplit = (matchId) => {
    console.log('handleMatchSplit called for:', matchId, 'scissorMode:', scissorMode);
    if (scissorMode) {
  ...
  const filteredMatches = matches.filter(match => {
    ...
    const matchesSearch = searchQuery === '' || 
      ...
      (match.person3 && match.person3.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const sidebarMatches = filteredMatches.filter(m => m.location === 'sidebar');
          ...
          <div className="sidebar-header">
            ...
            <div className="sidebar-actions">
              <button className="confirm-btn" onClick={handleConfirm}>
                confirm {selectedMatches.length > 0 && `(${selectedMatches.length})`}
              </button>
